...
# 用例依赖接口
def rely_func(rely_data):
	header = API_HEADER
	data = []
	for rely in rely_data:
		if rely['method'] == "post":
			if data:
				for post_data in data:
					rely['data']['orderno'] = post_data['orderno']
			rely_res = requests.post(API_URL + rely['uri'], json = rely['data'], headers = header)
			rely_res = rely_res.json()
			rely_list_data = copy.deepcopy(rely_res)
			for list_key in rely['list_key']:
				if not rely_list_data[list_key]:
					print("依赖URL：", API_URL + rely['uri'], "<br/><br/>")
					print("依赖请求数：", rely['data'], "<br/><br/>")
					print("请求头部：", header, "<br/><br/>")
					print("依赖数据没有返回任何值：", rely_res, "<br/><br/>")
					return []
				else:
					rely_list_data = copy.deepcopy(rely_list_data[list_key])

			ret = each_list(rely_list_data, rely)
			if not ret:
				print("依赖URL：", API_URL + rely['uri'], "<br/><br/>")
				print("依赖请求数：", rely['data'], "<br/><br/>")
				print("请求头部：", header, "<br/><br/>")
				print("没有满足条件的值：", rely_res, "<br/><br/>")
				return rely_res
			rely_one = random.choice(ret)
			data = []
			res = {}
			for key in rely['key']:
				for key_dict in key.keys():
					res[key[key_dict]] = rely_one[key_dict]
			data.append(res)
...
def each_list(list_name, rely):
	ret = []
	for data in list_name:
		if "where" in rely:
			for where_key in rely['where']:
				if where_key['term'] == ">":
					if data[where_key['keywords']] > where_key['value']:
						ret.append(data)
		else:
			ret.append(data)
	return ret
